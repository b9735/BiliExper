...
class asyncBiliApi(object):
    ...
    def __init__(self, headers: Optional[Dict[str, str]]):
        ...
        self._session = ClientSession(headers=headers, trust_env=True)

    async def login_by_cookie(
        self, cookieData, checkBanned=True, strict=False
    ) -> Awaitable[bool]:
        """
        登录并获取账户信息
    ...
    async def followUser(self, followid: int, type: int = 1):
        ...
        post_data = {"type": type, "follow": followid, "csrf_token": self._bili_jct}
    ...
    async def joinGroup(
        self, group_id: int, mobi_app: str = "web"
    ) -> Awaitable[Dict[str, Any]]:
        """
        加入应援团
        group_id    int  应援团id
    ...
    async def relationTagsAddUser(
        self, fids: int, tagids: int
    ) -> Awaitable[Dict[str, Any]]:
        """
        将用户加入关注分组
        fids    int  用户名称
        ...
        post_data = {"fids": fids, "tagids": tagids, "csrf": self._bili_jct}
        async with self._session.post(url, data=post_data, verify_ssl=False) as r:
            return await r.json()

    async def createRelationTag(self, tag: str) -> Awaitable[Dict[str, Any]]:
        """
        创建用户分组
        tag  str  分组名称
        """
        url = "https://api.bilibili.com/x/relation/tag/create"
        post_data = {"tag": tag, "csrf": self._bili_jct}
        async with self._session.post(url, data=post_data, verify_ssl=False) as r:
            return await r.json()

    async def getRelationByUid(self, uid: int) -> Awaitable[Dict[str, Any]]:
        """
        判断与某个up关系
        是否关注，关注时间，是否拉黑.....
        uid int up主uid
    ...
    async def getRelation(self, tagid: int = 0, pn: int = 1, ps: int = 50) -> dict:
        """
        取关注分组内up主列表
        tagid int 分组id
        """
    ...
    async def likeCv(self, cvid: int, type=1) -> Awaitable[Dict[str, Any]]:
        """
        点赞专栏
        cvid int 专栏id
        type int 类型
    ...
    async def vipPrivilegeReceive(self, type: int = 1) -> Awaitable[Dict[str, Any]]:
        """
        领取B站大会员权益
    ...
    async def getUserWallet(self, platformType: int = 3) -> Awaitable[Dict[str, Any]]:
        """
        获取账户钱包信息
    ...
    async def elecPayBcoin(self, uid: int, num: int = 5) -> Awaitable[Dict[str, Any]]:
        """
        用B币给up主充电
        uid int up主uid
        num int B币数量
    ...
    async def getDanmuInfo(self, roomid: int) -> Awaitable[Dict[str, Any]]:
        """
        查询直播间弹幕服务器
    ...
    async def xliveRoomInit(self, id: int = 1) -> Awaitable[Dict[str, Any]]:
        """
        获取房间初始化信息(房间短id转长id)
        id int 直播间id
        """
    ...
    async def xliveAnchorCheck(self, roomid: int) -> Awaitable[Dict[str, Any]]:
        """
        查询直播天选时刻
        roomid int 真实房间id，非短id
        """
    ...
    async def xliveAnchorJoin(
        self, id: int, gift_id: int, gift_num: int, platform: str = "pc"
    ) -> Awaitable[Dict[str, Any]]:
        """
        参与直播天选时刻
        id int 天选时刻id
    ...
    async def xliveBp2Gold(
        self, num: int = 5, platform: str = "pc"
    ) -> Awaitable[Dict[str, Any]]:
        """
        B币劵购买金瓜子
    ...
    async def xliveGetRoomInfo(self, room_id: int) -> Awaitable[Dict[str, Any]]:
        """
        B站直播获取房间信息
        room_id int 房间id
        """
    ...
    async def coin(
        self, aid: int, num: int = 1, select_like: int = 1
    ) -> Awaitable[Dict[str, Any]]:
        """
        给指定av号视频投币
        aid int 视频av号
    ...
    async def coinCv(
        self, cvid: int, num: int = 1, upid: int = 0, select_like: int = 1
    ) -> Awaitable[Dict[str, Any]]:
        """
        给指定cv号专栏投币
        cvid int 专栏id
    ...
    async def articleViewInfo(self, cvid: int) -> Awaitable[Dict[str, Any]]:
        """
        获取专栏信息
        cvid int 专栏id
        """
    ...
    async def xliveWebHeartBeat(
        self, hb: str = None, pf: str = None
    ) -> Awaitable[Dict[str, Any]]:
        """
        B站直播间心跳
    ...
    async def xliveHeartBeatX(
        ...
        async with self._session.post(
            enc_server, json={"t": post_data, "r": secret_rule}, verify_ssl=False
        ) as r:
    ...
    async def report(
        self, aid: int, cid: int, progres: int
    ) -> Awaitable[Dict[str, Any]]:
        """
        B站上报视频观看进度
        aid int 视频av号
        ...
        post_data = {"aid": aid, "cid": cid, "progres": progres, "csrf": self._bili_jct}
        async with self._session.post(url, data=post_data, verify_ssl=False) as r:
            return await r.json()

    async def share(self, aid) -> Awaitable[Dict[str, Any]]:
        """
        分享指定av号视频
        aid int 视频av号
        """
    ...
    async def silver2coin(self) -> Awaitable[Dict[str, Any]]:
        ...
        post_data = {"csrf_token": self._bili_jct, "csrf": self._bili_jct}
        async with self._session.post(url, data=post_data, verify_ssl=False) as r:
            return await r.json()

    async def getRegions(self, rid=1, num=6) -> Awaitable[Dict[str, Any]]:
        """
        获取B站分区视频信息
        rid int 分区号
        num int 获取视频数量
    ...
    async def mangaClockIn(self, platform="android") -> Awaitable[Dict[str, Any]]:
        """
        模拟B站漫画客户端签到
        platform str 平台
        """
    ...
    async def mangaShopExchange(
        self, product_id: int, point: int, product_num=1
    ) -> Awaitable[Dict[str, Any]]:
        """
        漫画积分商城兑换
        product_id int 商品id
    ...
    async def mangaGetVipReward(self) -> Awaitable[Dict[str, Any]]:
        ...
        async with self._session.post(
            url, json={"reason_id": 1}, verify_ssl=False
        ) as r:
            return await r.json()

    async def mangaComrade(self, platform="web") -> Awaitable[Dict[str, Any]]:
        """
        站友日漫画卷兑换查询
        platform str 平台
        """
    ...
    async def mangaPayBCoin(
        self, pay_amount: int, product_id: int = 1, platform: str = "web"
    ) -> Awaitable[Dict[str, Any]]:
        """
        B币购买漫画
        pay_amount int 购买数量
        ...
        """
        url = (
            f"https://manga.bilibili.com/twirp/pay.v1.Pay/PayBCoin?platform={platform}"
        )
        post_data = {"pay_amount": str(pay_amount), "product_id": product_id}
        async with self._session.post(url, json=post_data, verify_ssl=False) as r:
            return await r.json()

    async def mangaGetCoupons(
        self, not_expired=True, page_num=1, page_size=50, tab_type=1, platform="web"
    ) -> Awaitable[Dict[str, Any]]:
        """
        获取账户中的漫读劵信息
        not_expired bool
    ...
    async def mangaListFavorite(
        self, page_num=1, page_size=50, order=1, wait_free=0, platform="web"
    ) -> Awaitable[Dict[str, Any]]:
        """
        B站漫画追漫列表
        page_num int 页数
    ...
    async def mangaDetail(
        self, comic_id: int, device="pc", platform="web"
    ) -> Awaitable[Dict[str, Any]]:
        """
        获取漫画信息
        comic_id int 漫画id
    ...
    async def mangaGetEpisodeBuyInfo(
        self, ep_id: int, platform="web"
    ) -> Awaitable[Dict[str, Any]]:
        """
        获取漫画购买信息
        ep_id int 漫画章节id
    ...
    async def mangaAddFavorite(self, comic_id: int) -> Awaitable[Dict[str, Any]]:
        """
        将漫画添加进追漫列表
        comic_id int 漫画id
        """
    ...
    async def mangaAddHistory(
        self, comic_id: int, ep_id: int
    ) -> Awaitable[Dict[str, Any]]:
        """
        添加漫画观看历史
        comic_id int 漫画id
    ...
    async def activityAddTimes(
        self, sid: str, action_type: int
    ) -> Awaitable[Dict[str, Any]]:
        """
        增加B站活动的参与次数
        sid str 活动的id
        ...
        post_data = {"sid": sid, "action_type": action_type, "csrf": self._bili_jct}
        async with self._session.post(url, data=post_data, verify_ssl=False) as r:
            return await r.json()

    async def activityDo(self, sid: str, type: int) -> Awaitable[Dict[str, Any]]:
        """
        参与B站活动
        sid str 活动的id
        type int 操作类型
    ...
    async def activityMyTimes(self, sid: str) -> Awaitable[Dict[str, Any]]:
        """
        获取B站活动次数
        sid str 活动的id
        """
    ...
    async def getDynamic(
        self, offset_dynamic_id: int = 0, type_list=268435455
    ) -> Awaitable[Dict[str, Any]]:
        """取B站用户动态数据"""
    ...
    async def getDynamicDetail(self, dynamic_id: int) -> Awaitable[Dict[str, Any]]:
        """
        获取动态内容
        dynamic_id int 动态id
        """
    ...
    async def dynamicReplyAdd(
        self, oid: int, message="", type=11, plat=1
    ) -> Awaitable[Dict[str, Any]]:
        """
        评论动态
        oid int 动态id
    ...
    async def dynamicLike(
        self, dynamic_id: int, like: int = 1
    ) -> Awaitable[Dict[str, Any]]:
        """
        点赞动态
        dynamic_id int 动态id
    ...
    async def dynamicRepost(
        self, dynamic_id: int, content="", extension='{"emoji_type":1}'
    ) -> Awaitable[Dict[str, Any]]:
        """
        转发动态
        dynamic_id int 动态id
    ...
    async def dynamicCreate(
        self, content: str, ctrl: Iterable[Mapping[str, str]] = ()
    ) -> Awaitable[Dict[str, Any]]:
        """
        ...
        post_data = {
            ...
            "at_uids": ",".join(
                [u["data"] for u in ctrl if "type" in u and u["type"] == 1]
            ),  # @的用户的uid，多个用逗号(,)隔开
    ...
    async def getSpaceDynamic(
        self, uid: int = 0, offset_dynamic_id: int = ""
    ) -> "dict":
        """
        取B站空间的动态列表
        uid int B站用户uid
    ...
    async def removeDynamic(self, dynamic_id: int) -> Awaitable[Dict[str, Any]]:
        """
        删除自己的动态
        dynamic_id int 动态id
        """
    ...
    async def getLotteryNotice(self, dynamic_id: int) -> Awaitable[Dict[str, Any]]:
        """
        取指定抽奖信息
    ...
    async def getLotteryInfoWeb(self, room_id: int) -> Awaitable[Dict[str, Any]]:
        """
        取直播间抽奖信息
        room_id int 直播间id
        """
    ...
    async def StormCheck(self, room_id: int) -> Awaitable[Dict[str, Any]]:
    ...
    async def juryKpi(self) -> Awaitable[Dict[str, Any]]:
        """
        取当前账户风纪委员统计信息
        """
        url = "https://api.bilibili.com/x/credit/v2/jury/kpi"
        async with self._session.get(url, verify_ssl=False) as r:
            return await r.json()

    async def juryApply(self) -> Awaitable[Dict[str, Any]]:
        """
        申请加入风纪委员会
        """
    ...
    async def juryCaseOpinion(
        self, case_id: int, pn: int = 1, ps: int = 20
    ) -> Awaitable[Dict[str, Any]]:
        """
        获取一个案件的众议观点
        pn int 页码
    ...
    async def juryCaseInfo(self, case_id: int) -> Awaitable[Dict[str, Any]]:
        """
        获取风纪委员案件详细信息
        """
    ...
    async def juryVote(
        self, case_id: int, **kwargs  # 非必选参数太多以可变参数列表传入
    ) -> Awaitable[Dict[str, Any]]:
        """
        风纪委员投票
        case_id int 案件ID
        ...
        post_data = {"case_id": case_id, "csrf": self._bili_jct, **kwargs}  # 所有可选参数
        async with self._session.post(url, data=post_data, verify_ssl=False) as r:
            return await r.json()
    ...
    async def getRoomIdByUid(self, uid: int) -> Awaitable[Dict[str, Any]]:
        """通过uid获得直播间id
        uid  int  用户uid
        """
